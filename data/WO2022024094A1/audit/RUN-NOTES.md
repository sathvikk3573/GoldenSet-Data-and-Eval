...
### Prompt 7 re-run: independent final gold check (2026-09-01, fresh session)

A second Prompt 7 pass, run in a session that shared no context with the fixes,
addressing caveat 3 of the audit README. All 20 records re-derived field by field
against the enriched .md; every procedure_text re-verified verbatim by
segment-match (all segments are page-break or section-header stitches, zero gap
characters); all Example quantities, yields, purity bounds, melting points,
workup steps and addition orders re-checked against paragraphs [0047]-[0053] and
found exact. Nine fixes, F-032 to F-040, all recorded in the affected records'
notes:

- **F-032** (records 5, 6, 7): `no_conditions` removed — stale after F-003 filled
  `conditions.temperature` from the drawn arrows.
- **F-033** (record 8): procedure_text opened `[0009]` where line 88 prints
  `[009]`; corrected to verbatim.
- **F-034** (record 2): base identifier unified — reactant_names printed
  `(CH3CH2 )3N` while compounds[] printed `triethylamine`; now `triethylamine`
  in both with the printed formula kept as an alias. Two stale note claims
  superseded.
- **F-035** (records 13, 14): DCC added to reactant_names, matching Example 6 and
  Summary Step 2 for the identical coupling.
- **F-036** (record 16): sodium cyanide added to reactant_names, matching Summary
  Step 3. Triethylamine stays excluded on both, per the documented
  'in presence of' convention.
- **F-037** (record 17): the [0032] oxidant alternatives removed from
  reactant_names — the merge union had reintroduced the co-presence assertion
  F-020 fixed in compounds[].
- **F-038** (record 17): contradictory `workup_type:none` tag removed (the merge
  union carried it alongside `workup_type:filtration` and a non-empty workup).
- **F-039** (records 17, 18, 19): procedure_summary rewritten — the kept
  Summary-section sentence ("names no reagent, no solvent...") contradicted the
  merged records' own contents.
- **F-040** (record 9): chlorine added to reactant_names, removing the file's one
  excluded oxidant-role compound.

Two things checked and deliberately left, both already documented conventions:

1. **Compound-level `purity_pct` holds the printed lower bound as a number**
   (95.0/92.0/85.0) while the record level holds it as `product_purity_pct_min`
   after F-011. The compound-level form matches A1/compounds.json and its raw_text;
   changing it would break cross-artifact agreement. The tension is real and is
   recorded here rather than resolved.
2. **reactant_names has no single role-based rule.** The operative convention,
   now applied consistently: substrates plus charged, consumed co-reagents
   (oxidants, acids-in-medium, additives, coupling reagents, cyanide); catalysts,
   solvents, media, quench water and workup acids stay out; 'in presence of'
   bases stay out (records 16, 19), while record 2's drawn base stays in as its
   note documents. CN112645853A includes bases in reactant_names, so the two
   patents differ on bases; a scorer must read this note.

After these fixes the file is declared gold by this pass: 20 records, every
patent-stated value present, nothing invented, empty fields verified silent,
generated fields internally consistent.

### Prompt 8 re-run: independent final gold check on compounds.json (2026-09-01, fresh session)

All 52 records re-derived field by field against the enriched .md. Mechanical
sweeps: every identifier and every alias verified literal in the patent (zero
misses); id/uuid uniqueness; sections arrays vs roles_by_section; all 33 charges
in quantities_by_section re-verified against lines 264-329; every melting point,
appearance, purity bound and yield re-checked verbatim; cross-file role agreement
against reactions.json. Nine fixes, C-010 to C-018, all annotated in the affected
records' notes:

- **C-010** (NMSBA, enol ester): every `role_in_that_example` said 'product' —
  copied from the record-level role — where the charge is the compound going INTO
  a later step as reactant (Examples 4-6, Example 7). Only the two role-switching
  compounds were affected; roles_by_section already had it right.
- **C-011** (enol ester): singular quantity filled (20 g / 60 mmol, Example 7) —
  C-006's own rule keeps it on the four once-charged compounds and this was the
  fourth, left null.
- **C-012** (enol ester): the C-001 note said the backward-compatibility singular
  fields keep "Example 4's values"; they keep Example 6's (157-163 °C, off white
  colour solid). Note corrected.
- **C-013** (mesotrione, enol ester): isolated masses (17 g; 25.4/0.89/22.8 g)
  added to yields_by_section — printed beside each yield, previously held only in
  reactions.json.
- **C-014** (water): second Example 7 entry for the 60 mL wash portion, which
  existed only in prose notes.
- **C-015** (NMST, NMSBA, enol ester, ruthenium oxide): missing second-role
  entries added to roles_by_section (Background reactant roles on line 64; RuO2
  as product of the in-situ prep at [0034]; NMST as 'other' at line 192).
- **C-016** (NMSBC, 1,2-dichloroethane, dichloropropanes/butanes/pentanes,
  'an oxidant', 'a base', 'a solvent'): sections/roles_by_section arrays added —
  these 8 records also span multiple sections; C-008's fix had covered only the
  13 Example-linked compounds.
- **C-017** (alkali metal hypochlorite): generic_term:true added, matching
  'peroxide' from the same [0032] sentence.
- **C-018** ('a base'): compound_class:reagent → compound_class:base, matching
  its role, the seven named bases, and the sibling generic 'a solvent'.

Checked and deliberately left, reported to the user:

1. **HPPD's identifier_type 'iupac'** — an enzyme name is IUBMB nomenclature, not
   IUPAC chemical nomenclature, and every other resolved-false record is typed
   'other'; but the string is a systematic name and the file has no second enzyme
   to compare. Left as is; both readings recorded.
2. **chlorine role 'reagent'** vs reactions.json's 'oxidant' on the catalyst-prep
   step — the discrepancy is already RECORDED in the reactions record's notes as
   deliberate.
3. **Singular melting_point/appearance keep one batch's values** on the two
   multi-batch products — documented backward-compatibility convention, with the
   arrays as the full picture.

After these fixes the file is declared gold by this pass: 52 records, every
patent-stated value present in a field, nothing invented, aliases all literal,
generated fields internally consistent.
