# Prompt 9: identify every route the patent describes
...
- **One route told many times, or many routes?** A route stated in the Claims, the
  ...
  the ordered set of reaction ids; see Prompt 10.)
...
Keep that ledger **on disk**, not only in context, so a lost context or a slip never costs
more than the current chunk. Create `audit/` if needed and write it to
`audit/_pathways-identification.md`, appending as each chunk closes; on start, if it already
exists, resume from the last section marked done rather than re-reading from the top. It is
a transient working file — delete it once the route count is committed, so the finished
`audit/` stays clean.
