...
a file where most fields were never looked at. The columns, exactly:
    ...
    #, Reaction, Patent step, Line (English), Field, Compound or item,
    Role in the source, What the source says (English), What reactions.json has,
    Status, "Problem, in plain words"

- **#** numbers the rows from 1, contiguously.
- **Reaction** is the `reaction_id`. **Patent step** is what the patent itself calls
  it, in the source language.
- **Line (English)** is the line number in the `.md` where the evidence sits. Every
  row must carry one; a row with no line number cannot be checked.
- **What the source says (English)** quotes or tightly paraphrases the patent.
  **What reactions.json has** states the value actually in the file. Reading the two
...
- **`audit/reactions_audit.csv` is the record of this pass.** Every flag is a
  `MISSING` or `WRONG` row there, written so a beginner with no context can understand
  what is wrong: what the patent says, what the record has, what it should be, and the
  line number. The final gold check will pass a verdict on every one of these findings,
  so a finding that is not written down is a finding that never gets closed.
...
- `audit/reactions_audit.csv`, complete: every record covered, every field-check a
