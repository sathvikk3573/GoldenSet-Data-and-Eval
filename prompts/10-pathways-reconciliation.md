# Prompt 10: reconcile your route list against pathways.json

Only after Prompt 9 is finished and the route count is written down.
...
- **Duplicate restatements to merge.** Same route = same ordered reaction-id set (after
  ...
  them is the described-routes-vs-performed-runs convention you settled in Prompt 9 —
  apply it, do not decide it afresh here. The merge itself happens in Prompt 11.
...
- **Projection freshness.** Every pathway step is meant to be a verbatim copy of its gold
  ...
  was corrected — and Prompt 11 must re-project it from the gold before any content is
...
where `Raised in pass` is `Prompt 10`, `File` is `pathways.json`, `Line (.md)` is the
...
check (Prompt 11) fills the verdict and resolution on every one of these rows.
