# Prompt 12: verify, merge and gold-check pathways.json

The final pathways pass. Run this after Prompts 10 and 11, and after `reactions.json` and
...
looked like *before* the reactions gold passes corrected them. If Prompt 11's freshness
...
4. **Verify every field, deciding first where its answer comes from:**
   ...
   **b) Fields that must AGREE WITH THE GOLD** — `pathways.json` is not independent of the
   ...
   - **The `smiles` slot on `ksm` / each `intermediate` / `product` is usually null and
     usually fillable.** The gold `reactions.json` already carries `reactant_smiles`,
     `product_smiles` and `canonical_rxn`; the compound is IUPAC-named. Backfill the
     SMILES from the gold reaction that makes or consumes that compound (match on
     `compound_uuid` / identity), or by resolving the IUPAC name, and say which source you
     used. A SMILES that is resolvable but left null is a finding; one that cannot be
     resolved is correctly null, and say so.
...
Execute the `Duplicate` findings from Prompt 11. Edit in place — do not leave a `.bak`
...
1. **Build `audit/pathways_audit.csv` as you go**, one row per field checked, passes
   included. Same shape as the reactions sheet:
...
2. **Close the pathways findings in `audit/findings.csv`**: one row for every pathway
   finding raised in Prompts 11 and this pass — every `pathways.json` row of
