...
4. **Then read the patent `.md`, keeping that specific record in mind** — its own section
   and every section it cross-references, using the `<!-- page ... :: <section_type> -->`
   ...
   carefully' actually means", and do that on every pass. A line can concern your record
   without naming it: "the salt prepared in step (1)", "the same procedure as Example 1",
   a structure on an arrow, or the line that fixes what R or M stands for. Read for
   anything related, not only for direct mentions.
...
there are 55 records, that is 55 readings. But "read the patent" means the record's own
section plus the sections it cross-references, not the whole file 55 times: on a large
...
## Merge the duplicate restatements

This is the pass that executes the `Duplicate` findings Prompt 2 raised — the same
transformation carried by several records across Claims, Summary, Example and Scheme
sections must collapse to one. Do this once, deliberately, and record it.

1. **Do not leave a backup behind.** Edit in place; the audit trail and your logged
   reasoning are the record. If you must snapshot before the bulk edit, delete the
   snapshot once you have verified the result — the finished folder carries no `.bak`.
2. **Confirm each group before collapsing.** Same product and reactant set after
   normalisation, and the duplicates a strict generic subset of the survivor (no
   conflicting temperature, reagent or yield). Any conflicting specified value means a
   genuinely different run — do NOT merge it. A distinct route's step (a thioether
   variant, say) is its own transformation — keep it.
3. **Keep the richest record as the survivor** (most non-null conditions/quantities/
   yield, usually the worked Example), and **union into it any non-null field a dropped
   duplicate carries that the survivor lacks** — the survivor wins every conflict. Never
   invent or alter a chemical fact, and preserve flagged patent defects exactly as
   printed.
4. **Keep `source_sections`** on the survivor — the distinct `section_label`s the
   reaction was drawn from, including the survivor's own. **Never write a `merged_from`
   field** (or any other merge-bookkeeping key) onto a record: the gold does not carry
   it, and the provenance of the merge lives in `findings.csv`, not in the json.
5. **Do not write a separate merge ledger.** There is no `dedup_reactions.csv`; the
   merge is closed as its `Duplicate` finding in `findings.csv` (step below), whose
   resolution names the survivor and the records folded into it.
6. **Update any stated record count** you keep in a note with a dated line.
7. **Verify:** the file still parses as a list; no record carries a `merged_from` key;
   and the surviving record count == original_count − the number of records folded away.

Every `Duplicate` finding then gets its verdict in `findings.csv` below, the same as
any other finding.

## Close the audit folder

This pass finishes the patent's `audit/` folder. Two obligations:

1. **Bring `audit/reactions_audit.csv` up to date with the corrected file.** Every row
...
2. **Close every finding in `audit/findings.csv`.** For every reaction finding raised
   in Prompts 2 and 5 — every reactions row of `findings.csv` (including every
   `Duplicate` row now merged), and a fresh row for every `MISSING`/`WRONG` row of
   `audit/reactions_audit.csv` — fill the `Verdict` and `Resolution / evidence`
   columns. None skipped: a finding with no verdict is a finding that was silently
   dropped. The shared findings schema:

       #, Raised in pass, File, Line (.md), Finding, Status when raised, Verdict,
         Resolution / evidence

   - **Verdict** is `NO LONGER TRUE` (the corrected file now has it right), `STILL TRUE`
     (still open — say what is blocking it), or `NEVER TRUE` (the finding itself was
     mistaken — say why).
   - **Resolution / evidence** points at the current file and the patent: the field,
     the value now present, the line number, so the verdict is checkable from its own
     row. For a merged `Duplicate`, name the survivor and the records folded into it.
...
- The finished audit folder: `audit/reactions_audit.csv` reflecting the corrected file,
  and the reaction rows of `audit/findings.csv` with a verdict on every finding. No
  `dedup_reactions.csv`, no `findings_verdicts.csv`, no `.bak`.
