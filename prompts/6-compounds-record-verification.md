...
2. **Read the patent `.md` with respect to that compound**, line by line, end to end.
   ...
   description, what it is called in the original language and in English, and how it is referred to
   indirectly.
...
a file where most fields were never looked at. The columns, exactly:

    #, Compound, Section, Line (English), Field, Role in the source,
    What the source says (English), What compounds.json has,
    Status, "Problem, in plain words"

- **#** numbers the rows from 1, contiguously.
- **Compound** is the record's `identifier`. **Section** is where the evidence lives
  (Example 1, claims, a table, a drawing).
- **Line (English)** is the line number in the `.md`. Every row must carry one; a row
  with no line number cannot be checked.
- **What the source says (English)** quotes or tightly paraphrases the patent.
  **What compounds.json has** states the value actually in the file. Reading the two
  side by side must be enough to see whether they agree.
- **Status** is one of: `OK`, `MISSING` (patent says it, file does not), `WRONG`
  (values disagree), `NOTE` (a documented convention or merge, nothing lost),
  `EXCLUDED` (deliberately out of scope, with the reason).
- **Problem, in plain words** is written for a beginner with no context: what the
  patent says, what the record has, what it should be. For an `OK` row a short
...
- **`audit/compounds_audit.csv` is the record of this pass.** Every flag is a
  `MISSING` or `WRONG` row there, written so a beginner with no context can understand
  it: what the patent says, what the record has, what it should be, and the line
  number. The final gold check will pass a verdict on every one of these findings, so
  a finding that is not written down is a finding that never gets closed.
...
- `audit/compounds_audit.csv`, complete: every record covered, every field-check a
  row, passes as well as failures.
