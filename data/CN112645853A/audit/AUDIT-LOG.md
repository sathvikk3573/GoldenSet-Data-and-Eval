...
## Blind adversarial review, compounds.json (2026-09-01, cross-patent pass)
A fresh no-context subagent (barred from audit/ and reactions.json; notes treated as
claims to verify) reviewed all 56 records against the full patent. Its numeric
verdict: every stored quantity, all 22 product assays, all 12 in-process salt values
and both impurity ranges verbatim-correct; batch capture complete; no invented or
missing quantities. It reported 1 CRITICAL + 8 MAJOR + 5 MINOR findings. Disposition
after independent verification (backup CN112645853A-compounds.json.before-blindreview
in the session scratchpad):
FIXED (17 records):
 - CRITICAL: record 40 benzyl alcohol resolved->false. The 苄醇 of [0024] (L182) is
   the substrate's own benzylic alcohol (the drawn ether by-product, L185
   C18H16Cl2O9S2, is two substrate units, not (PhCH2)2O); the sibling benzyl-bromide
   record's note had already named this resolution an error.
 - tembotrione + tefuryltrione commercially_available false->true (their own notes
   and the mesotrione sibling say true; [0004] L130-131).
 - tefuryltrione: the TRANSLATION CONFLICT note claiming the EN line renders the name
   fenquinotrione was removed as factually wrong — the string appears nowhere in the
   source; L266 reads tembotrione and tefuryltrione. Fabricated claim corrected.
 - records 34/36: the generic salt string removed from aliases (it sat on three
   records; the generic record owns it; nothing joins through it — verified against
   reactions.json); functional_group:carboxylic_acid -> carboxylate;
   +chemical_family:benzyl_halide (the salts keep the CH2Br).
 - records 5/6 is_section_product true->false (their own Technical-Field segments
   assign the flag to the generic acid, record 0 — now the sole carrier);
   record 6 -compound_class:active_ingredient (patent: key intermediate, L132).
 - record 39 tags unified with its three alkoxide siblings (alkoxide family,
   -compound_class:reagent).
 - record 49 alias 或ROM removed (或 is the conjunction "or" in the drawn condition,
   L176). Stray null smiles keys removed from 12 records (shape uniformity).
 - Note-side corrections (fields already right): record 9 analytics-vs-note
   contradiction resolved in favour of the entries; record 1's stale "none is
   emitted" claim superseded (base/acid/ROH/ROM records exist; [0007]'s 相应的醇/醇钠
   fold into the existing generic identities per identity dedup); records 35/37
   "recorded as a reactant" clauses superseded (role product is the uniform sibling
   convention).
REFUTED: the claimed off-by-one line citation on record 34 (the None token does open
line 317 — the reviewer misread); the tertiary-alcohol/basic-substance coverage
asymmetry left as a coverage-convention observation, no field wrong.
Post-fix battery clean: 56 records, ids/uuids unique, all reaction-side references
still resolve, generic salt string single-owner, is_section_product coherent.
compounds.json: GOLD after this pass.

## Blind adversarial review, reactions.json (2026-09-01, cross-patent pass)
A fresh no-context subagent reviewed all 55 records against the full patent.
Data-layer verdict: 100% — every charge, condition, yield, assay and quoted text
correct; mass-balance flags reproduced independently; zero CRITICAL findings.
8 MAJOR + 12 MINOR generated-layer findings; disposition after verification
(backup CN112645853A-reactions.json.before-blindreview in session scratchpad):
FIXED (32 records): Ex8 2a step_role first->intermediate + missing distillation tag
+ stale mechanism note clause; aqueous solvent tag corrected on four step-2 records
(removed from Ex7, which charges solid NaOtBu L405; added to Ex3/15/16, which charge
30% aqueous hydroxide L331/533/543, matching Ex6); Ex5 2b catalyst_class base->none
(pre-formed alkoxide, no base, as the seven siblings) + two false note clauses
superseded; workup_type:distillation added to the nine step-2 records whose own
workup prints 负压脱溶 untagged; purification:none added to the five step-2 records
missing it; the eight inherited Ex13-20 Step 1 records' tag sets extended to match
the Example 1 content the gold correction copied into them (+ the phantom
linkage:cross_section_linked note claim superseded on five); Ex9/10 Step 1 promoted
to one-pot with the alkoxide formation enumerated + transformation:salt_formation +
reagent_written_not_drawn, matching Ex11/12 — the patent prints the identical
滴入…溶解完全后 sequence and the hydride-alcohol reaction is as real as the metal's
(schemes L431/L449 put only the hydride on the arrow); Ex17 2a catalyst_class
none->base (hydride prep, as Ex1/5/18); solvent_class:neat added to Ex1/2/5/8 2a
(alcohol as reagent-and-medium, as Ex17-20); safety:pyrophoric_reagent added to the
two other performed KH charges (Ex18 2a, Ex10 S1); stale note clauses superseded on
Summary_Step 2, Claims_Step 1 and Claims_Step 2.
REFUTED: the analytics:hplc step-1/step-2 asymmetry (the tag mirrors
process_control.method exactly, i.e. in-process monitoring — verified zero
mismatches); the methanol by_product placement on Background_Step 1 (documented
judgment, patent-linked). Post-fix battery: all groups uniform, mirrors clean.
reactions.json: GOLD after this pass.
ADDENDUM: post-fix battery exposed one more note-vs-record miss of the same class —
Background_Side Reaction's note states step_role is side_reaction but the record
carried no step_role tag; step_role:side_reaction added (every other record carries
exactly one). Final battery clean.
