# Prompt 9: adversarial independent audit by fresh no-context agents

Phase 4. Run this LAST, after Prompts 7 and 8, when both files are already believed to
be gold. Everything before this was done by one mind that has now read the patent a
hundred times and cannot unsee what it found. This pass hands the files to people who
have never seen them, and asks them to prove the gold wrong.

The value of this pass is the same as the value of Prompts 1 and 3: **independence**.
...
stopped being surprised by it. Do not skip this pass on the assumption that Phases 1-3
were thorough. They were, and it still found things.

## What you (the main agent) do

You have full context: you ran Prompts 1-8, you know every decision. **That is exactly
...
1. **Spawn the finders, in parallel, one file each.** At minimum two, run at the same
   time: one against `output/reactions.json`, one against `output/compounds.json`. They
   do not talk to each other and they do not see each other's output.
...
- **You may read exactly two things:** the patent (the single `*-enriched-numbered.md` in `input/`),
  and the one file you are auditing (`output/reactions.json` OR `output/compounds.json`,
  never both).
- **You are forbidden from opening the `audit/` folder, the other json file, or any
...
1. **Judge it against the patent and the current file**, exactly as Prompt 7 judges a
   field: is it real? Go to the line, read it, decide.
...
1. **Update `audit/reactions_audit.csv` and `audit/compounds_audit.csv`.** Every value
   you corrected on this pass becomes a `FIXED` row (old value, new value, line number in
   "Problem, in plain words"). A field a finder checked that had no row yet gets one, so
   the sheets still cover the whole file. After this pass no row may describe a value the
   file no longer holds.

2. **Append the Prompt 9 findings to `audit/findings.csv`**, one row per finding each
   ...
   - **Raised in pass** names the finder (`Prompt 9 reactions finder`, `Prompt 9
     original-document check`). **Verdict** is `NO LONGER TRUE` / `STILL TRUE` /
...
The file is not gold while any Prompt 9 verdict says `STILL TRUE`.
...
- **Run the finders in parallel.** The reactions file and the compounds file are
...
- The finished audit folder: both `*_audit.csv` sheets reflecting any corrections, and the
  Prompt 9 rows of `audit/findings.csv` with a verdict on every finding.
